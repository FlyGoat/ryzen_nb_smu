...
u32 nb_reg_read(struct pci_dev *nb, u32 addr)
{
    u32 val;
    pci_write_config_dword(nb, NB_PCI_REG_ADDR_ADDR, (addr & (~0x3)));
    pci_read_config_dword(nb, NB_PCI_REG_DATA_ADDR, &val);
    return val;
}

void nb_reg_write(struct pci_dev *nb, u32 addr, u32 data)
{
    pci_write_config_dword(nb, NB_PCI_REG_ADDR_ADDR, addr);
    pci_write_config_dword(nb, NB_PCI_REG_DATA_ADDR, data);
}
